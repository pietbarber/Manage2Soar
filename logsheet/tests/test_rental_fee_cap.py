...
from logsheet.models import Glider, Flight, Logsheet, Airfield, Towplane
...
def test_rental_cost_no_cap():
    ...
    flight = Flight.objects.create(
        pilot=member, glider=glider, logsheet=logsheet,
        launch_time=time(10, 0), landing_time=time(13, 0)
    )
...
def test_rental_cost_below_cap():
    ...
    airfield = Airfield.objects.create(
        identifier="KFRY", name="Below Cap Field")
    ...
    flight = Flight.objects.create(
        pilot=member, glider=glider, logsheet=logsheet,
        launch_time=time(10, 0), landing_time=time(12, 0)
    )
    flight.refresh_from_db()
    assert flight.rental_cost == Decimal("120.00")
