...
from logsheet.models import Logsheet, LogsheetCloseout, Towplane, TowplaneCloseout, Airfield
...
def test_delete_button_not_shown_for_finalized_logsheet(client, active_member):
    ...
    assert f"action=\"{reverse('logsheet:delete', args=[logsheet.pk])}\"" not in response.content.decode(
    )


@pytest.mark.django_db
def test_delete_button_not_shown_for_logsheet_with_towplane_closeout(client, active_member):
    airfield = Airfield.objects.create(name="Test Field")
