...
class TestLogsheetPermissions(TestCase):
    ...
    def test_regular_member_cannot_unfinalize(self):
        """Test that regular members cannot unfinalize logsheets they didn't finalize"""
        self.logsheet.finalized = True
        self.logsheet.save()

        # Create a revision log showing that someone else finalized it
        # (duty officer finalized it, but regular member is trying to unfinalize)
        RevisionLog.objects.create(
            logsheet=self.logsheet,
            revised_by=self.duty_officer,
            note="Logsheet finalized"
