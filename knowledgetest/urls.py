...
urlpatterns = [
    ...
    path(
        "attempt/<int:pk>/delete/",
        WrittenTestAttemptDeleteView.as_view(),
        name="quiz-attempt-delete",
    ),
    path("pending/", PendingTestsView.as_view(), name="quiz-pending"),
    path(
        "history/", MemberWrittenTestHistoryView.as_view(), name="member-test-history"
    ),
    path(
        "instructor/recent/",
        InstructorRecentTestsView.as_view(),
