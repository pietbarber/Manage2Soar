...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        cursor.execute("SELECT handle, role_name, is_qualified, expiration_date, instructor, notes FROM quals")
        rows = cursor.fetchall()

        for handle, role_name, is_qualified, expiration_date, instructor_handle, notes in rows:
            try:
            ...
            if instructor_handle:
                try:
                    instructor = Member.objects.get(legacy_username__iexact=instructor_handle)
                except Member.DoesNotExist:
                    logger.warning(f"Instructor '{instructor_handle}' not found for qualification '{role_name}' on '{handle}'.")
