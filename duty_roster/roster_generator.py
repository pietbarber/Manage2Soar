...
def generate_roster(year=None, month=None):
    ...
    def eligible(role, m, day, assigned):
        ...
        eligible_role_fields = [field for r, field in percent_fields if getattr(m, r, False)]
        if len(eligible_role_fields) == 1:
            field = eligible_role_fields[0]
            pct = getattr(p, field, 0)
            if pct == 0:
                logger.debug(f"{m.full_display_name}: Only eligible for {role}, percent is 0, treating as 100%.")
                pct = 100
        else:
            ...
            if all_zero:
                logger.debug(f"{m.full_display_name}: All eligible role percents are zero, treating {role} as 100%.")
        if not flag:
            logger.debug(f"{m.full_display_name}: Not eligible for role {role} (flag is False).")
            return False
        if pct == 0:
        ...
        if assignments[m.id] >= getattr(p, 'max_assignments_per_month', 0):
            ...
            return False
        logger.debug(f"{m.full_display_name}: Eligible for {role} on {day} (pct={pct}).")
        return True
        ...
        for m in cands:
            ...
            for o in assigned:
                if o.id in pairings.get(m.id, set()) or m.id in pairings.get(o.id, set()):
                    w *= 3
                    break
            logger.debug(f"Candidate: {m.full_display_name}, base weight: {base}, final weight: {w}")
            weights.append(w)
        if not weights or sum(weights) == 0:
            logger.debug(f"No candidates with nonzero weights for role {role}.")
            return None
        chosen = random.choices(cands, weights=weights, k=1)[0]
        logger.debug(f"Chose {chosen.full_display_name} for role {role} from {[m.full_display_name for m in cands]}")
        return chosen
    schedule = []
