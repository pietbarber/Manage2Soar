...
class InstructionRequestWindowTests(TestCase):
    ...
    def setUp(self):
        ...
        # Import SiteConfiguration here to keep it local and avoid circular issues
        from siteconfig.models import SiteConfiguration

        self.config = SiteConfiguration.objects.first()
        if not self.config:
    ...
    def _make_assignment(self, days_ahead):
        """Create a DutyAssignment that many days in the future."""
    ...
    def test_helper_disabled_by_default(self):
        ...
        self.assertFalse(self.config.restrict_instruction_requests_window)
        ...
        self.assertIsNone(opens_on)

    def test_helper_within_window(self):
    ...
    def test_helper_outside_window(self):
        ...
        too_early, opens_on = _check_instruction_request_window(far_date)
        self.assertTrue(too_early)
        expected_opens_on = far_date - timedelta(days=14)
        self.assertEqual(opens_on, expected_opens_on)

    def test_helper_exactly_at_boundary(self):
    ...
    def test_helper_past_date_never_too_early(self):
    ...
    def test_post_blocked_outside_window(self):
        ...
        self.assertEqual(response.status_code, 200)
        # No InstructionSlot should have been created
        from duty_roster.models import InstructionSlot
        ...
        # Error message should mention the open date
        opens_on = future_date - timedelta(days=14)
        self.assertContains(response, opens_on.strftime("%B"))

    def test_post_allowed_within_window(self):
    ...
    def test_post_allowed_when_restriction_disabled(self):
        """POST succeeds for a far-future date when restriction is disabled."""
        # restriction is disabled by default
        self.assertFalse(self.config.restrict_instruction_requests_window)

        assignment, future_date = self._make_assignment(60)
        self.client.login(username="window_student", password="testpass123")
