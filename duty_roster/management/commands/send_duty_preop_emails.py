...
from logsheet.models import MaintenanceIssue, MaintenanceDeadline
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument('--date', type=str, help='Target date for pre-op report (YYYY-MM-DD)')
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.NOTICE(f"Generating pre-op report for {target_date}"))
        ...
        upcoming_deadlines = MaintenanceDeadline.objects.filter(due_date__lte=target_date + timedelta(days=30))

        lines = [f"🚨 Pre-Operations Summary for {target_date}", ""]

        lines.append("👥 Assigned Duty Crew:")
        lines.append(f"🎓 Instructor: {assignment.instructor.full_display_name if assignment.instructor else '—'}")
        lines.append(f"🎓 Surge Instructor: {assignment.surge_instructor.full_display_name if assignment.surge_instructor else '—'}")
        lines.append(f"🛩️ Tow Pilot: {assignment.tow_pilot.full_display_name if assignment.tow_pilot else '—'}")
        lines.append(f"🛩️ Surge Tow Pilot: {assignment.surge_tow_pilot.full_display_name if assignment.surge_tow_pilot else '—'}")
        lines.append(f"📋 Duty Officer: {assignment.duty_officer.full_display_name if assignment.duty_officer else '—'}")
        lines.append(f"💪 Assistant DO: {assignment.assistant_duty_officer.full_display_name if assignment.assistant_duty_officer else '—'}")
        lines.append("")
        ...
        if to_emails:
            ...
            self.stdout.write(self.style.WARNING("⚠️ No valid email addresses for duty crew. Email not sent."))
